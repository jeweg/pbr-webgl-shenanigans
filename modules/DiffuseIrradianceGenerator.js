...
constructor(resolution) {
    ...
    for (let i = 0; i < 2; ++i)
    {
        this.renderTargets.push(new THREE.WebGLCubeRenderTarget(resolution, {
            format: THREE.RGBAFormat,
            magFilter: THREE.LinearFilter,
            minFilter: THREE.LinearFilter,
            type: THREE.HalfFloatType,
        }));
        this.cameras.push(new THREE.CubeCamera(0.1, 10, this.renderTargets[i]));
    }
    ...
    this.dummy = new THREE.WebGLCubeRenderTarget(resolution, {
            format: THREE.RGBAFormat,
            magFilter: THREE.LinearFilter,
            minFilter: THREE.LinearFilter,
            type: THREE.HalfFloatType,
        });
    this.camera = new THREE.CubeCamera(0.1, 10, this.dummy);
    */
    
    //this.camera = new THREE.CubeCamera(0.1, 10, this.renderTargets[1]);
    ...
    */
    ...
    this.passCount = Math.ceil(totalSamples / maxSamplesPerPass);

    console.log('total samples:', totalSamples, ', #passes: ', this.passCount);
...
\n\
            void main()\n\
            {\n\
                //gl_FragColor = vec4(1,100,10000,0.5) * 1000.f; return; \n\
...
\n\
                ...
                for (int samplei = 0; samplei < MAX_SAMPLES_PER_PASS; ++samplei)\n\
                {\n\
                    float realSampleIndex = float(samplei) + sampleOffset;\n\
...
\n\
    ...
    update(renderer, inputCubeMap)
    {
        ...
        for (var passIndex = 0; passIndex < this.passCount; ++passIndex)
        {
            this.material.uniforms['u_passIndex'].value = passIndex;

            // Switches source and destination render targets
            const currentRTIndex = passIndex % 2;
            const lastPassRTIndex = 1 - currentRTIndex;

            this.material.uniforms['u_lastPassOutput'].value = this.renderTargets[lastPassRTIndex].texture;
            this.cameras[currentRTIndex].update(renderer, this.scene);
            last_texture = this.renderTargets[currentRTIndex].texture;
        }
        return last_texture;
    }
}

export { DiffuseIrradianceGenerator };